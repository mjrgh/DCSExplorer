...
// --------------------------------------------------------------------------
//
// Utilties for translated ADSP-2105 aseembly code implementations of 
// the decoder
//

// ADSP-2105 system register simulation.  This handles writes to the
// special memory locations DM($3FE0..$3FFF) mapped as control registers.
// Note that the actual ADSP-2105 registers have many more functions
// besides the ones implemented here; we only implement the ones that
// the DCS decoder needs for the purposes of this simulation.  We don't
// even implement recognition of all of the bits that the DCS code sets,
// because some of those refer to hardware details that aren't relevant
// in the translated version (e.g., bus wait states when accessing
// external ROM), while others are simply inherent in the way this
// program works because the DCS code *always* uses them, thus there's
// no need to make them "options".
//
void DCSDecoderTranslated::WriteControlReg(uint16_t addr, uint16_t data)
{
	switch (addr)
	{
	case 0x3FFF:
		...
		if ((data & 0x0200) != 0)
		{
			// bit $0200 (BFORCE) performs a soft reset
			SoftBoot();

			// a reset requires re-entering the code from the beginning,
			// so throw a reset exception to unwind the host stack back
			// to the simulated machine entrypoint
			throw DCSDecoder::ResetException();
		}
		break;
	...
	case 0x3FEF:
		...
		if ((data & 0x0002) != 0)
		{
			// SPORT1 enabled - set the autobuffer parameters.  Note that
			// this doesn't actually start the autobuffer - that requires
			// a write to TX1.

			// get the Ix and Mx register assignments
			int ireg = (data >> 9) & 7;
			int mreg = ((data >> 7) & 3) | (ireg & 0x04);

			// remember the buffer parameters
			autobuffer.Set(&DM[Ix(ireg)], Lx(ireg), Mx(mreg));
		}
		break;
	}
}

void DCSDecoderTranslated::StoreROMPointer(uint16_t dmAddr, ROMPointer rp)
{
	uint32_t ofs = static_cast<uint32_t>(rp.p - ROM[rp.chipSelect].data);
	uint32_t chipSelectBits = rp.chipSelect << (hwVersion == HWVersion::DCS95 ? 21 : 20);
	uint32_t linearAddr = chipSelectBits | ofs;

	DM[dmAddr] = static_cast<uint16_t>((linearAddr >> 16) & 0xFFFF);
	DM[dmAddr + 1] = static_cast<uint16_t>(linearAddr & 0xFFFF);
}
